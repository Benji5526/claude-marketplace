...
### 목 규칙(정규식/키워드)을 짤 때 흔히 나오는 버그

목 모드는 정확도가 중요하지 않다고 해서 아무렇게나 짜면, 실제 LLM이라면 안 할 실수를 목이 저지르고 그게 다시 "왜 이렇게 나왔지"라는 디버깅 시간을 잡아먹는다. 자주 나오는 패턴 네 가지:

1. **키워드가 겹치는 카테고리는 우선순위를 정한다.** 예: "hotel 사진 보여줘"에 "hotel"과 "photo"가 둘 다 있으면, `if (/hotel/) ... else if (/photo/)` 순서라면 항상 hotel 쪽이 이긴다. 더 좁고 구체적인 신호(가격, 미디어 요청처럼 목적이 뚜렷한 것)를 먼저 검사하고, 넓은 키워드(hotel, trip처럼 다른 문맥에도 흔히 등장하는 단어)는 나중에 검사한다.
2. **너무 넓은 단독 키워드는 오탐을 만든다.** "stay"만으로 숙박 추천 의도를 잡으면, "How long should I stay in Bangkok?"(체류 기간을 물은 것뿐)도 걸려버린다. 의미가 겹치는 넓은 단어는 더 구체적인 단어(hotel, accommodation, resort)로 좁히거나 뺀다.
3. **후보 목록에서 고를 때는 "배열 순서"가 아니라 "텍스트 등장 순서"로 고른다.** 여러 지명/카테고리 후보가 있는 목록에서 `.find()`로 첫 매치를 고르면, 문장에 여러 후보가 동시에 등장할 때 사용자가 실제로 말한 순서가 아니라 코드에 적어둔 배열 순서가 이긴다. 각 후보의 `indexOf` 위치를 비교해서 텍스트상 가장 먼저 나온 것을 고르는 편이 실제 발화 의도에 더 가깝다.
4. **정규식은 사용자가 실제로 쓸 법한 여러 표현을 커버해야 한다.** 숫자 하나를 뽑을 때도 "3 people", "3 pax", "3 of us", "3 friends"(이 경우 본인을 더해 4명), 통화 표현도 "$2000", "2000 dollars", "budget is 2000" 등 여러 형태가 나온다. 목 모드를 만들 때 자기 자신이 낼 법한 문장 몇 개로만 테스트하지 말고, 같은 정보를 다른 말투로 표현한 변형을 최소 2~3개씩 넣어서 확인한다.
...
2. 목 규칙은 겹치는 키워드가 섞인 문장(예: "hotel 사진 보여줘")과, 같은 정보를 다르게 표현한 문장들로 짧은 단위 테스트를 짜서 확인한다 — 위 "흔히 나오는 버그" 네 가지를 각각 최소 하나씩 케이스로 만든다.
3. 별도 프로세스로 같은 사용자 ID를 대상으로 여러 턴을 실행해, DB 기반 메모리가 프로세스를 넘어 이어지는지 확인한다 (진짜 "다음 세션에서도 기억하는지"를 검증하는 가장 현실적인 방법).
4. 키를 채운 뒤 실제 호출로 1~2턴을 확인한다. 429가 뜨면 호출 횟수부터 의심한다 — 재시도 로직보다 호출 통합이 먼저다.
